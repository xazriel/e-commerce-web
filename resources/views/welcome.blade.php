...
<body class="antialiased bg-white text-gray-900" x-data="{ loginModal: false }">
    ...
    <section id="koleksi" class="py-24 max-w-7xl mx-auto px-4">
        ...
        <div class="flex flex-wrap justify-center gap-8 mb-20">
            <button onclick="filterCategory('all')" class="category-btn active text-[10px] uppercase tracking-[0.2em] pb-2 text-gray-400 hover:text-black transition">All Products</button>
            @foreach($categories as $cat)
                <button onclick="filterCategory('{{ $cat->slug }}')" class="category-btn text-[10px] uppercase tracking-[0.2em] pb-2 text-gray-400 hover:text-black transition">{{ $cat->name }}</button>
            @endforeach
        </div>
        
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-x-6 gap-y-12">
            @forelse($products as $product)
                <a href="{{ route('product.details', $product->slug) }}" class="group block no-underline product-item" data-category="{{ $product->category->slug }}">
    ...
    <div x-show="loginModal" 
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div x-show="loginModal"
                 ...
                 class="relative bg-white w-full max-w-md p-10 shadow-2xl z-10">
                
                <div class="flex justify-between items-center mb-10">
                    <h2 class="text-xs tracking-[0.4em] uppercase font-bold text-black">Login to Farhana</h2>
                    <button @click="loginModal = false" class="text-gray-400 hover:text-black focus:outline-none">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" required autofocus
                               class="w-full border-b border-gray-200 focus:border-black outline-none py-2 text-sm tracking-widest">
                    </div>
                    ...
                    <div x-data="{ show: false }">
                        ...
                        <div class="relative">
                            ...
                            <button type="button" 
                                    @click="show = !show" 
                                    class="absolute right-0 top-2 text-gray-400 hover:text-black focus:outline-none">
                                ...
                                <svg x-show="show" x-cloak class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7 1.274-4.057 5.064-7 9.542-7 1.254 0 2.438.271 3.5.755M21 21l-5.22-5.22m0 0A3 3 0 1010.78 10.78m5.22 5.22l4.22 4.22m-4.22-4.22l-5.22-5.22m3.93-2.017A3 3 0 0012 9h0c-.483 0-.939.114-1.343.315M3 3l3.59 3.59" />
                                </svg>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="pt-4">
                        <button type="submit" class="w-full bg-black text-white py-4 text-[10px] tracking-[0.3em] uppercase hover:bg-gray-800 transition duration-300">
                            Sign In
                        </button>
                    </div>

                    <div class="text-center mt-6">
                        <a href="{{ route('register') }}" class="text-[9px] tracking-[0.2em] uppercase text-gray-400 hover:text-black transition">
                            Create Account
                        </a>
                    </div>
                </form>
        ...
        function filterCategory(category) {
            ...
            buttons.forEach(btn => {
                if ((category === 'all' && btn.textContent.toLowerCase().includes('all')) || 
                    btn.textContent.toLowerCase().trim() === category.replace('-', ' ').toLowerCase()) {
